...
export default function LaptopForm() {
    ...
    const timeout = setTimeout(async () => {
      ...
      try {
        const { data } = await axios.get(
          `/api/laptops/checkName?name=${encodeURIComponent(normalized)}`
        );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.name.trim())
      return Swal.fire("Error", "Laptop name is required", "error");
    if (nameExists)
      return Swal.fire("Error", "Laptop name already exists!", "error");
    if (!form.imageBase64)
      return Swal.fire("Error", "Laptop image is required", "error");
  ...
  const handleAutofill = async () => {
    if (!form.name.trim()) {
      return Swal.fire("Error", "Enter a laptop name first", "error");
    }

    try {
      setLoading(true);
      const { data } = await axios.post("/api/fill-laptop", {
        laptopName: form.name,
      });
  ...
  return (
    <form className="max-w-3xl mx-auto p-4 space-y-6" onSubmit={handleSubmit}>
      <h2 className="text-2xl font-bold text-center">Add New Laptop</h2>
      ...
      <div className="mt-1 h-5 text-sm">
        ...
        {!checkingName && nameExists && (
          <span className="text-red-600">⚠️ Laptop name already exists!</span>
        )}
        {!checkingName && form.name.trim() && !nameExists && (
          <span className="text-green-600">✅ Laptop name is available</span>
        )}
      </div>
      <button
        type="button"
      ...
      <button
        ...
        disabled={loading}
      >
        {loading ? "Submitting..." : "Submit Laptop"}
      </button>
    </form>
  );
}
